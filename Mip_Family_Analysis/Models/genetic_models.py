...
class ModelChecker(Process):
    """Check which genetic models that are followed in a batch of variants."""
    
    def __init__(self, lock, task_queue, results_queue, family, verbose = False):
    ...
    def run(self):
        ...
        while True:
            ...
            for gene, variants in next_batch.items():
                compound_candidates = []
                compound_pairs = []
                if gene != '-':
                    compound_candidates = [variant for variant_id, variant in self.check_compound_candidates(variants).items()]
                    if len(compound_candidates) > 1:
                        compound_pairs = self.check_compound(compound_candidates)
                    else:
                        compound_pairs = []
    ...
    def check_compound_candidates(self, variants):
    ...
    def check_compound(self, list_of_variants):
        ...
        # Check in all individuals what genotypes that are in the trio based of the individual picked.
            for individual in self.family.individuals:
                ...
                if individual.phenotype == 2:# The case where the individual is affected
                    ...
                    mother_phenotype = self.family.get_phenotype(mother_id)
                    ...
                    father_phenotype = self.family.get_phenotype(father_id)
...
#         
#         # These list are to reduce the number of compounds to be checked:
#         hetero_both_parents = [] # This group will be checked against all others, not to itself
#         hetero_mother_only = [] # This group will be checked against all others, not to itself
#         hetero_father_only = [] # This group will be checked against all others, not to itself
#         others = [] # This group will be checked against all
#     
#         # Prepare for compounds by adding all variants for each gene:
#         if check_compound_candidate(variant, family):
#             
#             for gene_id in variant.genes:
