...
def score_variant(variant_object, genetic_models):
    ...
    check_predictions(variant_object)
    check_functional_annotation(variant_object)
    check_gene_annotation(variant_object)
    check_frequency_score(variant_object)
    check_filter(variant_id)
    check_region_conservation(variant_object)
    check_base_conservation(variant_object)
    check_phylop_score(variant_object)
    check_segmental_duplication(variant_object)
    check_hgmd(variant_object)
    check_inheritance(variant_object)
...
def check_frequency_score(thousand_genomes_frequency = None, dbsnp_frequency = None, dbsnp_id = None):
    ...
    freq_scores.append(get_freq_score(db_snp_frequency))
    common = False
    ...
    if common:
        ...
        frequency_score += sum(freq_score) / 2
...
def check_filter(filt):
    ...
    filter_score = 0
    if filt == 'PASS':
        self.variants[variant_id].rank_score += 3
    elif filt == 'PRES':
        self.variants[variant_id].rank_score += 1
    return filter_score
    
def check_region_conservation(mce64way = None, gerp_conserved = None):
    """Score the variant based on what annotations it has for the region conservations"""
    region_conservation_score = 0
    if mce64way != '-' and gerp_conserved != '-':
        region_conservation_score += 2
    elif mce64way != '-' or gerp_conserved != '-':
